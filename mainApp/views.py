...
def checkoutDetails(request):
	...
	try:
		...
		if request.method == "POST":
			...
			ch.address2 = request.POST.get('address1')
...
def signupUser(request):
	...
	if choice == 'seller':
		...
		pwd = request.POST.get('password')
		try:
			user = User.objects.create_user(username = s.uname, email = s.email, password = pwd)
			s.save()
		...
		pwd = request.POST.get('password')
		try:
			user = User.objects.create_user(username = b.uname, email = b.email, password = pwd)
			b.save()
			messages.success(request,'Account Created!! Please try to login')
			return HttpResponseRedirect('/login/')
		except:
...
def editProduct(request,num):
	...
	if request.method == "POST":
		...
		if request.POST.get('xl') == "1":
			p.xl = False
		if not request.FILES.get('img1') is None:
			p.img1 = request.FILES.get('img1')
		if not request.FILES.get('img2') is None:
			p.img2 = request.FILES.get('img2')
		if not request.FILES.get('img3') is None:
			p.img3 = request.FILES.get('img3')
		if not request.FILES.get('img4') is None:
			p.img4 = request.FILES.get('img4')
		if not request.FILES.get('img5') is None:
			p.img5 = request.FILES.get('img5')
		p.seller = s
		p.save()
		return HttpResponseRedirect('/profile/')
